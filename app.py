...
# Projeção baseada em ritmo real de avanço por faixa
#   • Já graduadas → definitivo
#   • Bucket 4 (Quase lá) → já estão na última faixa, contam
#   • Quem avançou de faixa: projeta o ritmo (buckets/semana) até 12/07
#       Se o bucket projetado >= 4 (Quase lá ou além), conta como provável formada
def projetada_a_graduar(p, weeks_left):
    if p["graduado"]:
        return True
    if p["bucket_atual"] == 4:
        return True  # já está na faixa 75-99%, falta só fechar
    hist = p["historico"]
    if len(hist) < 2:
        return False  # sem histórico, não dá pra projetar
    first_bucket = bucket_of(hist[0]["progresso"], False)
    advance = p["bucket_atual"] - first_bucket
    if advance <= 0:
        return False  # não avançou de faixa, não vai chegar
    # Ritmo real (buckets por semana) com base no histórico observado
    first_d = datetime.strptime(hist[0]["semana"], "%Y-%m-%d").date()
    last_d  = datetime.strptime(hist[-1]["semana"], "%Y-%m-%d").date()
    weeks_elapsed = max(1.0, (last_d - first_d).days / 7)
    rate = advance / weeks_elapsed  # ex: 0,33 buckets/semana
    projected_bucket = p["bucket_atual"] + rate * weeks_left
    return projected_bucket >= 4  # vai chegar em "Quase lá" ou além

proj_form = sum(1 for p in div_ppl if projetada_a_graduar(p, weeks_left))
