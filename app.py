...
# Logo PdA inline (estrela de 8 pontas estilizada)
LOGO_SVG = f"""
<svg width="56" height="56" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0">
  <g transform="translate(50,50)">
    <rect x="-30" y="-30" width="60" height="60" fill="none" stroke="{YELLOW}" stroke-width="6" />
    <rect x="-30" y="-30" width="60" height="60" fill="none" stroke="{YELLOW}" stroke-width="6" transform="rotate(45)" />
...
# Projeção: "Quase lá" + Formados = candidatos a fechar
proj_form = sum(1 for p in div_ppl if p["bucket_atual"] == 4 or p["graduado"])
    ...
    <div class="cmp-row">
      <div class="cmp-header">
        <div>
          ...
          <div class="cmp-sublabel">{sublabel}</div>
        </div>
        <div class="cmp-delta" style="color:{color}">{icon} Δ {sign}{delta:.1f} p.p.</div>
      </div>
      <div class="cmp-bar-row">
        <div class="cmp-name div">🎯 Diversidade</div>
        <div class="cmp-bar"><div style="width:{div_w:.1f}%;background:{YELLOW}">{div_pct:.1f}%</div></div>
      </div>
      <div class="cmp-bar-row">
        <div class="cmp-name nao">⚪ Não-Diversidade</div>
        <div class="cmp-bar"><div style="width:{nao_w:.1f}%;background:{GRAY}">{nao_pct:.1f}%</div></div>
      </div>
    </div>
    """
...
funnel_html = '<div style="background:'+CARD_BG+';border:1px solid '+CARD_BORDER+';border-radius:12px;padding:24px">'
for i in range(5):
    bn = i + 1
    info = BUCKET_INFO[bn]
    curr = funnel_curr[i]
    ...
    funnel_html += f"""
    <div class="funil-row">
      <div class="funil-label">{info['label']}<small>{info['range']} do bootcamp</small></div>
      <div class="funil-bar"><div style="width:{w:.1f}%;background:{info['color']}">{curr:,}</div></div>
      <div class="funil-delta {d_class}"><span class="abs">{sign}{d_abs}</span><span class="pct">{d_pct:.1f}% sem ant.</span></div>
    </div>
    """.replace(",", ".")

funnel_html += f"""
<div style="margin-top:18px;padding:12px 16px;background:rgba(245,166,35,.12);border-left:3px solid {ORANGE};border-radius:6px;font-size:12px;color:{ORANGE}">
  💡 <strong>Foco prioritário:</strong> as {funnel_curr[3]} pessoas em "🟠 Quase lá" são as candidatas mais prováveis de fechar a meta até 12/07.
</div>
</div>
"""
...
for p in prioridades[:10]:
    ...
    <div class="contact-card">
      <div style="display:grid;grid-template-columns:1fr 100px;gap:18px;align-items:center">
        <div>
          <div class="contact-name">{p['nome']} {tier_badge_html(p['tier'])}</div>
          <div class="contact-meta">📧 {p['email']} · 📱 {fmt_wpp(p['whatsapp'])} · faixa: {bucket_info['range']} · {mov}</div>
        </div>
        <div>
          <div class="contact-score">{p['score']}</div>
          <div style="font-size:10px;color:{GRAY};text-align:right">score conversão</div>
        </div>
      </div>
      <details style="margin-top:10px">
        <summary style="cursor:pointer;font-size:11px;color:{GRAY}">📝 ver mensagem sugerida</summary>
        <div class="msg-box">{generate_message(p)}</div>
      </details>
    </div>
    """, unsafe_allow_html=True)
...
for b in [1, 2, 3, 4, 5]:
    fig_evo.add_trace(go.Scatter(
        x=evo_dates_fmt, y=evo_data[b],
        name=f"{BUCKET_INFO[b]['label']} ({BUCKET_INFO[b]['range']})",
        stackgroup="one",
        line=dict(width=0.5, color=BUCKET_INFO[b]['color']),
        fillcolor=BUCKET_INFO[b]['color'],
...
fig_evo.update_layout(
    ...
    xaxis=dict(gridcolor="#5a1060", color=GRAY),
    ...
    with tab:
        ...
        for idx, p in enumerate(ppl[:200]):
            ...
            <tr>
              ...
              <td>{p['email']}<br><span class="td-meta">{fmt_wpp(p['whatsapp'])}</span></td>
              <td>{bucket_pill_html(p['bucket_atual'])}</td>
              <td>{movimento_qualitativo(p)}</td>
              <td>{tier_badge_html(p['tier'])}</td>
            </tr>
            """
        if len(ppl) > 200:
            rows_html += f'<tr><td colspan="6" style="text-align:center;color:{GRAY};padding:14px">... + {len(ppl) - 200} pessoas (use o download XLSX para a lista completa)</td></tr>'
