...
def load_history() -> dict:
    # Session state survives st.rerun() and is the primary source
    if "history" in st.session_state:
        return st.session_state["history"]
    # Fallback: try reading from disk (first load of a new session)
    try:
...
def save_history(history: dict):
    # Always save to session_state first — survives reruns even if disk write fails
    st.session_state["history"] = history
    # Try to persist to disk as a best-effort backup
    try:
        os.makedirs(os.path.dirname(STORAGE_PATH), exist_ok=True)
        with open(STORAGE_PATH, "w", encoding="utf-8") as f:
            json.dump(history, f, ensure_ascii=False, indent=2)
    except Exception:
        pass  # disk write failed silently — session_state still has the data
...
def generate_message(p: dict) -> str:
    ...
    if tier == "Alta Prioridade":
        ...
        return (
            ...
            f"Você se inscreveu no Bootcamp, isso quer dizer que quer dar um passo a mais na sua carreira! 🧑🏾‍💻\n\n"
            f"A gente sabe que aprender sozinho(a) é difícil, por isso a PdA oferece monitorias síncronas gratuitas "
            f"para te apoiar onde precisar, viu? 🤗\n"
        ...
        return (
            f"Oi, {nome}! 👋\n\n"
            f"A porta do Bootcamp Automação de Dados com IA, uma parceria entre DIOxAfyaxPdA, "
            f"ainda está aberta até 12 de julho. Nunca é tarde para começar! 📆\n\n"
            f"Você se inscreveu no Bootcamp, isso quer dizer que quer dar um passo a mais na sua carreira! 🧑🏾‍💻\n\n"
            f"A gente sabe que dar o primeiro passo sozinho(a) pode ser difícil, por isso a PdA oferece "
            f"monitorias síncronas gratuitas para te apoiar desde o início, viu? 🤗\n"
            f"Acesse nossa comunidade no WhatsApp: {WPP_LINK}\n\n"
            f"Junto a gente vai mais longe! 😉\n"
            f"Estamos torcendo por você! 💜"
        )
...
<div class="pda-header">
  <div>
    <h1>Bootcamp DIOxAfyaxPdA — Monitor de Formação</h1>
...
def process_uploads(files):
    ...
    any_new = False
    for f in files:
        # Skip files already processed in this session (avoids rerun loop)
...
nao_div_ppl   = [p for p in all_ppl if not p["diversidade"]]
nao_div_total = len(nao_div_ppl)
nao_div_vel   = mean([p["velocidade"] for p in nao_div_ppl])
nao_div_prog  = mean([p["progresso_atual"] for p in nao_div_ppl])
nao_div_accel = sum(1 for p in nao_div_ppl
                    if p["delta_ultimo"] is not None and p["delta_penult"] is not None
                    and p["delta_ultimo"] > p["delta_penult"])
nao_div_decel = sum(1 for p in nao_div_ppl
                    if p["delta_ultimo"] is not None and p["delta_penult"] is not None
                    and p["delta_ultimo"] < p["delta_penult"])
...
with col_donut:
    ...
    for tier, color in zip(donut_labels, donut_colors):
        ...
        st.markdown(
            ...
            f'<span style="color:{YELLOW};font-weight:600;min-width:36px;text-align:right;display:inline-block">{count}</span>'
            f'<span style="color:{GRAY};font-size:11px;min-width:28px;text-align:right;display:inline-block">{pct_t}%</span>'
            f'</div>',
            unsafe_allow_html=True
        )
...
with col_momentum:
    ...
    with st.expander("↔ Comparar com grupo geral (sem diversidade)"):
        ...
        div_accel_pct     = round(div_accel / div_total * 100) if div_total else 0
        div_decel_pct     = round(div_decel / div_total * 100) if div_total else 0
        nao_div_accel_pct = round(nao_div_accel / nao_div_total * 100) if nao_div_total else 0
        nao_div_decel_pct = round(nao_div_decel / nao_div_total * 100) if nao_div_total else 0
        for col, label, div_v, nd_v in [
            (c1, "Velocidade/sem",  f"+{div_vel*100:.1f}%",     f"+{nao_div_vel*100:.1f}%"),
            (c2, "Progresso médio", f"{div_prog*100:.0f}%",      f"{nao_div_prog*100:.0f}%"),
            (c3, "Aceleraram",      f"{div_accel_pct}% do grupo",f"{nao_div_accel_pct}% do grupo"),
            (c4, "Desaceleraram",   f"{div_decel_pct}% do grupo",f"{nao_div_decel_pct}% do grupo"),
        ]:
            with col:
                st.markdown(
                    ...
                    f'<div style="color:{GRAY}">👥 Não-div: {nd_v}</div>'
                    f'</div>',
                    unsafe_allow_html=True
                )
...
# FUNIL DE PROGRESSÃO
# ═══════════════════════════════════════════════════════════════════
st.markdown('<p class="section-title">📊 Funil de Progressão — Diversidade</p><p class="section-sub">Quantas pessoas atingiram cada marco · comparativo semana a semana</p>', unsafe_allow_html=True)

milestones = [0.25, 0.50, 0.75, 1.00]
mil_labels = ["25% · Início", "50% · Meio", "75% · Avançado", "100% · Concluído"]

funnel_curr = [sum(1 for p in div_ppl if p["progresso_atual"] >= m) for m in milestones]
...
if len(sorted_dates) >= 2:
    ...
    for i, m in enumerate(milestones):
        ...
        for p in div_ppl:
            prog_prev = None
            for h in sorted(p["historico"], key=lambda x: x["semana"]):
                if h["semana"] <= prev_date:
                    prog_prev = h["progresso"]
            if prog_prev is not None and prog_prev >= m:
                count += 1
        funnel_prev[i] = count

fig_funil = go.Figure()
fig_funil.add_trace(go.Bar(
    y=mil_labels, x=funnel_prev,
    name="Semana Ant.", orientation="h",
    marker_color="#555555",
    text=funnel_prev, textposition="outside", textfont=dict(color="#aaa", size=11),
))
fig_funil.add_trace(go.Bar(
    y=mil_labels, x=funnel_curr,
    name="Esta Semana", orientation="h",
    marker_color=YELLOW,
    text=funnel_curr, textposition="outside", textfont=dict(color=YELLOW, size=12),
))
...
rows_html = ""
for label, curr, prev in zip(mil_labels, funnel_curr, funnel_prev):
    d_abs = curr - prev
    d_pct = (d_abs / prev * 100) if prev else 0.0
    cor   = "#4ecb71" if d_abs > 0 else ("#e24b4a" if d_abs < 0 else "#888")
    sinal = "+" if d_abs >= 0 else ""
    rows_html += (
        f'<tr>'
        f'<td style="padding:10px 14px">{label}</td>'
        f'<td style="text-align:center;padding:10px 14px;color:#aaa">{prev}</td>'
        f'<td style="text-align:center;padding:10px 14px;color:{YELLOW};font-weight:700;font-size:15px">{curr}</td>'
        f'<td style="text-align:center;padding:10px 14px;color:{cor};font-weight:600">{sinal}{d_abs}</td>'
        f'<td style="text-align:center;padding:10px 14px;color:{cor};font-weight:600">{sinal}{d_pct:.1f}%</td>'
        f'</tr>'
    )

st.markdown(f"""
<table style="width:100%;border-collapse:collapse;background:#3a003d;border-radius:10px;overflow:hidden;font-size:13px;margin-bottom:16px">
  <thead>
    <tr style="background:#1a0020">
      <th style="padding:10px 14px;text-align:left;font-size:11px;font-weight:600;color:{YELLOW};text-transform:uppercase;letter-spacing:.5px">Etapa</th>
      <th style="padding:10px 14px;text-align:center;font-size:11px;font-weight:600;color:{YELLOW};text-transform:uppercase;letter-spacing:.5px">Semana Ant.</th>
      <th style="padding:10px 14px;text-align:center;font-size:11px;font-weight:600;color:{YELLOW};text-transform:uppercase;letter-spacing:.5px">Esta Semana</th>
      <th style="padding:10px 14px;text-align:center;font-size:11px;font-weight:600;color:{YELLOW};text-transform:uppercase;letter-spacing:.5px">Δ Abs.</th>
      <th style="padding:10px 14px;text-align:center;font-size:11px;font-weight:600;color:{YELLOW};text-transform:uppercase;letter-spacing:.5px">Δ %</th>
    </tr>
  </thead>
...
# ── FOOTER ────────────────────────────────────────────────────────────────────
st.markdown(f'<div style="text-align:center;padding:40px 0 20px;color:#5a1060;font-size:11px">Programadores do Amanhã · Monitor Bootcamp DIOxAfyaxPdA</div>', unsafe_allow_html=True)
