...
def generate_message(p: dict) -> str:
    ...
    if tier == "Alta Prioridade":
        return (
            f"Oi, {nome}! 👋\n\n"
            f"Você está na reta final do Bootcamp Afya e tem tudo para garantir sua certificação em "
            f"Automação de Dados com IA antes do encerramento em 12 de julho. Não para agora!\n\n"
            f"Você não precisa chegar lá sozinha. A PdA oferece monitorias síncronas de todo o conteúdo "
            f"do bootcamp. Acesse o grupo do WhatsApp e participe quando quiser: {WPP_LINK}\n\n"
            f"A PdA existe para acelerar pessoas negras e indígenas no mercado de tecnologia, e você já "
            f"está mostrando que tem o que precisa para chegar lá. Vai até o fim! 🚀"
        )
    elif tier == "Atenção":
        return (
            f"Oi, {nome}! 👋\n\n"
            f"Você está no Bootcamp Afya e ainda tem tempo de garantir sua certificação em "
        ...
        return (
            f"Oi, {nome}! 👋\n\n"
            f"Você está indo muito bem no Bootcamp Afya, parabéns pela dedicação!\n\n"
            f"Continue nesse ritmo e a certificação em Automação de Dados com IA é sua até 12 de julho. "
            f"E se quiser acelerar ainda mais, a PdA tem monitorias síncronas para você: {WPP_LINK}\n\n"
            f"A PdA está aqui para acelerar pessoas negras e indígenas no mercado de tecnologia. "
            f"Você está chegando lá! 🌟"
        )
    elif tier == "Estagnado":
        return (
            f"Oi, {nome}! 👋\n\n"
            f"Passamos para dar um alô e lembrar que o Bootcamp Afya ainda está aberto até 12 de julho.\n\n"
            f"A PdA oferece monitorias síncronas para te apoiar onde precisar. "
...
if len(sorted_dates) >= 2:
    def avg_prog_all(d, key):
        rows = [r for r in history[d] if (r["diversidade_negro"] or r["diversidade_indig"]) if key=="div" else True]
        if key == "all": rows = history[d]
        return mean([r["progresso"] for r in rows]) * 100
